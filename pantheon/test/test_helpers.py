...
def get_git_summary(mode='local', remote_path=None):
    ...
    local_git_summary = "not a git repo" #TODO Fix? check_output(git_summary_src, cwd=project_root.DIR)
    ...
    if mode == 'remote':
        ...
        remote_git_summary = "not a git repo" # TODO Fix? check_output(ssh_cmd, shell=True)
...
def get_recv_sock_bufsizes(ssh_cmd=None):
    ...
    local_max_bufsize = 0 #TODO Fix? get_kernel_attr(max_cmd)
    local_default_bufsize = 0 #TODO Fix? get_kernel_attr(default_cmd)

    buf_sizes['local']['max'] = local_max_bufsize
    buf_sizes['local']['default'] = local_default_bufsize

    if ssh_cmd is not None:
        remote_max_bufsize = 0 #TODO Fix? get_kernel_attr(max_cmd, ssh_cmd)
        remote_default_bufsize = 0 #TODO Fix? get_kernel_attr(default_cmd, ssh_cmd)
